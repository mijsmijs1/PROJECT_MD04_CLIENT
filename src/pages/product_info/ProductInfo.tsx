...
export default function ProductInfo() {
  ...
  product = productStore.product?.filter((item: any) => item.id == id)

  useEffect(() => {
    // if (!localStorage.getItem("token")) return
    const fetchData = async () => {
      try {
        const res = await api.authen.getUserById(Number(product[0]?.userId));
        if (res.status == 200) {
          dispatch(userAction.setUserProduct(Object.values(res.data.data)));
  ...
  const [mainImage, setMainImage] = useState(`${import.meta.env.VITE_SV_HOST}/${product[0]?.imgs[0]?.imgUrl}`);
  ...
  return (
    <div className='product_info_container'>
      <div className='product_info'>
        ...
        <div className='info_detail'>
          <div className='name'>
            ...
            <p>Tin đăng lúc: {formatTimeAgo(Number(product[0]?.createAt))} </p>
          </div>
          <div className="dashed">
            <div className="dashed-line"></div>
          </div>
          ...
          <div className='detail'>
            ...
            <p>{` ${product[0]?.address.replace(/&&/g, ", ")} `}</p>
      ...
      <div className='info'>
        ...
        {
          product[0]?.priorityStatus == "active" && <div className='policy'>
